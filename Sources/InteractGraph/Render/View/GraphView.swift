    ...
    internal var body: some View {
        ZStack {
            ForEach(edges) { edge in
                if let fromNode = nodeFrames[edge.from], let toNode = nodeFrames[edge.to] {
                    Path { path in
                        path.move(to: fromNode.bottomCenter)
...
//        graph.addEdges([edge0, edge1, edge2, edge3, edge4])
        ...
        graph.addNodes([node2, node3, node4])
        graph.addEdges([edge2, edge3, edge4])
